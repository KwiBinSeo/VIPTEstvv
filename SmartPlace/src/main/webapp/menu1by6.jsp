...
<%@page import="com.cglab.smartplace.AllData" %> <!-- 사용자가 입력하는 모든 데이터들을 저장할 객체 import -->
 ...
 <%
	...
	*/
	...
	int materials_list_count_s = count; // materials_list_count_s 변수 값을 count로 초기화(사급)
	// 세션 데이터 저장
	try {
		session.setAttribute("materials_list_count_s", materials_list_count_s); // materials_list_count_s 세션 객체 생성
	} catch (Exception e) {
		...
		<header class="main-header">
      			...
      			<ul class="sidebar-menu" data-widget="tree">
        			...
        			<li class="active treeview">
          				...
          				<ul class="treeview-menu">
							...
							<li class="active"><a href="/smartplace/menu1by6.jsp"><i class="fa fa-circle-o"></i>자재&nbsp;투입&nbsp;현황(사급)</a></li>
  		...
  		<div class="content-wrapper">
    		...
    		<section class="content-header">
	      		<h1> 자재 투입 현황(사급)<small>Material input status</small></h1>
	      		<ol class="breadcrumb">
			...
			<section class="content">
				<div class="col-md-12">
					<div class="box box-danger">
						<div class="box-body">
							<form id="frm" action="/smartplace/menu1by6Toby7.do" method="POST" >
								<!-- 추가 -->
										...
										<tbody>
											<% if(materials_input_quantity_s == null) {
												for(int index = 0; index < materials_list_count_s; index++) { 
													if(type[index].equals("사급") || type[index] == "사급") { %>
											<tr>
												...
												<td><center><label name="<%="materials_total"+index%>" for="<%=total[index]%>"><%=total[index]%></label>\
												<input type="hidden" name="<%="materials_total"+index%>" value="<%=total[index]%>"></input>
												<td><center><input type="text" name="<%="materials_base_quantity"+index%>" value="<%=base_quantity[index]%>"></input></td>
												<td><center><input type="text" name="<%="materials_input_quantity"+index%>" value="<%=input_quantity[index]%>"></input></td>
												<td><center><input type="text" name="<%="materials_total_quantity"+index%>" value="<%=total_quantity[index]%>"></input></td>
											...
											<tr>
												...
												<td><center><label name="<%="materials_total"+index%>" for="<%=total[index]%>"><%=total[index]%></label>
												<input type="hidden" name="<%="materials_total"+index%>" value="<%=total[index]%>"></input>
												<td><center><input type="text" name="<%="materials_base_quantity"+index%>" value="<%=materials_base_quantity_s.get(index)%>"></input></td>
												<td><center><input type="text" name="<%="materials_input_quantity"+index%>" value="<%=materials_input_quantity_s.get(index)%>"></input></td>
												<td><center><input type="text" name="<%="materials_total_quantity"+index%>" value="<%=materials_total_quantity_s.get(index)%>"></input></td>
												<td><center><input type="text" name="<%="materials_etc"+index%>" value="<%=etc[index]%>"></input></td>
											</tr>
											<% } else {}}} %>
										</tbody>
